    ...
    for await (const entry of Deno.readDir(PACKAGES_DIR)) {
        if (entry.isDirectory) {
            ...
            if (await exists(pkgDocsDir)) {
                ...
                const targetDocsDir = join(DIST_DIR, 'packages', entry.name, 'docs');
            ...
            if (await exists(pkgLlmsFile)) {
                console.log(`  - Syncing llms.txt for @lockness/${entry.name}...`);
                const targetLlmsFile = join(DIST_DIR, 'packages', entry.name, 'llms.txt');
