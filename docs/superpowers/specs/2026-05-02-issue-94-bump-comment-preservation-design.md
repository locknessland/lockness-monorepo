...
The per-package `packages/<name>/deno.json` files do not have this problem.
They are plain JSON by convention (verified across `core`, `cache`, `events`,
and others — none contains a comment), so `JSON.parse` / `JSON.stringify`
behaves identically to a comment-preserving editor for them. The bug is
narrowly scoped to the root `deno.jsonc` rewrite path.
...
3. New behaviour is fixture-tested so future refactors can't silently
   regress it.
...
- Splitting into `scripts/jsonc_edit.ts` would add a module for one consumer;
  YAGNI per the runbook's "single, narrow responsibility" guidance reads as
  one helper, not one file.
...
`packages/*/deno.json` files have no comments by repo convention, and Deno's
own `deno publish` workflow assumes plain JSON for package configs. Forcing
the heavier CST path everywhere would buy nothing and add a parse cost per
package. Note this decision in the script header so a future contributor who
wants to add a comment to a package config gets the right signal (add a doc
test that asserts package configs are comment-free, or migrate that one path
...
1. `tests/bump.test.ts` gains a fixture-based test that:
   - Reads `tests/fixtures/bump/deno.jsonc.input` (a deliberately
     comment-rich, formatting-quirky JSONC).
...
3. Smoke test (manual or scripted in the test): run the live bump script
   against a temp copy of the real root `deno.jsonc` to a throw-away version,
   diff against original — only `version` and `@lockness/*` lines differ.
   Cover at least `core`, `cache`, `events` package files in the same run.
4. `deno fmt && deno lint && deno check scripts/bump.ts tests/bump.test.ts &&
   deno task test` all green (CLAUDE.md rule #5).
...
- `/Users/kevin/Sites/lockness/tests/bump.test.ts` — add fixture-based tests
  for the new helper.
