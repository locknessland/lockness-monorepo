...
replacing the traditional `bootstrap()` function:
...
- `@Kernel(config)`: Declares database, session, devtools, controllers, and
  middleware configuration
- `@DeclareGlobalMiddleware()`: Marks a property as the global middleware stack
- `@OnBoot({ priority })`: Lifecycle hooks executed during startup (higher
  priority runs first)
- `createApp(KernelClass)`: Bootstraps the application from the decorated kernel

### Externalized Configuration

Lockness encourages **externalized configuration** in the `config/` directory at
the project root. This keeps the kernel clean and makes configuration testable
and reusable.
...
├── app/
│   ├── auth/
│   │   └── guards.ts   # Auth guards configuration
│   ├── middleware/
│   │   └── i18n_middleware.ts
│   └── kernel.tsx      # Uses config from ../config/
└── main.ts
```
...
export const databaseConfig = {
    url: Deno.env.get('DATABASE_URL') || 'postgres://localhost:5432/mydb',
}
```
...
- **Clean Kernel**: The kernel stays declarative without inline configuration
- **Testable**: Config objects can be mocked or overridden in tests
- **Type-safe**: Full TypeScript support with proper types
- **Organized**: Each concern has its own file
- **Reusable**: Config can be imported anywhere in the application
...
- The `@Kernel` decorator handles both modes automatically:

```typescript
@Kernel({
    controllersDir: './app/controller',  // Used in development
    controllers: controllers,             // Used in production
})
export class AppKernel { ... }
```
...
- **`packages/`**: Contains the decoupled libraries. This modularity allows for
  ...
  `lockness-drizzle`.
- **`config/`**: Externalized configuration files for database, session, i18n,
  and app settings. Keeps the kernel clean and configuration testable.
