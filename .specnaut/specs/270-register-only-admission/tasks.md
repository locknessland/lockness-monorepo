...
- [ ] T001 Rebase `270-register-only-admission` onto `origin/main`.
...
- [ ] T002 **Suite baseline.** Run `deno test -A packages/realtime/` and save the output to the scratchpad. It must be
...
- [ ] T004 **The migration list, re-measured** (plan D1, FR-013).
...
- [ ] T005 [P] Migrate `packages/realtime/tests/authorize_result_357.test.ts` (84 failing in the probe).
- [ ] T006 [P] Migrate `packages/realtime/tests/authorize_result_347.test.ts` (55).
- [ ] T007 [P] Migrate `packages/realtime/tests/presence_member_id_type_346.test.ts` (47).
- [ ] T008 [P] Migrate `packages/realtime/tests/manager_debt_353.test.ts` (34).
- [ ] T009 [P] Migrate `packages/realtime/tests/lapse_rehold_349.test.ts` (28) and
  `packages/realtime/tests/presence_member_admission_350.test.ts` (28).
- [ ] T010 [P] Migrate `packages/realtime/tests/presence_member_frozen_354.test.ts` (18) and
  `packages/realtime/tests/channel_watch_295.test.ts` (16).
- [ ] T011 [P] Migrate `packages/realtime/tests/revocation_paging_359.test.ts` (13) and
  `packages/realtime/tests/channel_revoke_332.test.ts` (10).
- [ ] T012 [P] Migrate `packages/realtime/tests/disconnect_admission_361.test.ts` (12: W1, W2, W4, W5 and the eight W6
  cases; plan D3). Register first. Their assertions do not change.
- [ ] T013 [P] Migrate the 5–9 group in `packages/realtime/tests/`: `presence_sweep_departure_348`, `churn_cost_329`,
...
- [ ] T014 [P] Migrate the 2–4 group in `packages/realtime/tests/`: `channels`, `leave_outcome_332`, `presence`,
...
- [ ] T015 [P] Migrate the 1-each group in `packages/realtime/tests/`: `connection_id_charset`, `control_auth`,
...
- [ ] T016 **The premise list.** Name, by file and test, every test left unchanged under the phase rule. Record the
  list in this task when you tick it. Phase 9 rewrites exactly these tests.
- [ ] T017 **The proof, then commit 1.**
...
- [ ] T018 New file `packages/realtime/tests/register_only_admission_370.test.ts` with its harnesses:
...
- [ ] T019 Write W1–W9 and W11–W13 in `packages/realtime/tests/register_only_admission_370.test.ts`, exactly as in the
  plan's §4 table.
...
- [ ] T020 Add **`#361 W13`** (the post-site before the caps) to
  `packages/realtime/tests/disconnect_admission_361.test.ts` (plan A3). It is a pin: run it green on the current
  tree.
- [ ] T021 Add `ConnectionNotRegisteredError` to `packages/realtime/manager.ts`, after `ConnectionIdInUseError`.
  **Row 6's home.**
...
- [ ] T022 [US1] Add `#assertBound(connection)` below `#assertAdmissible` in `packages/realtime/manager.ts`. **Row 3's
...
- [ ] T023 [US1] In `subscribe`, in `packages/realtime/manager.ts`, replace both `this.#assertAdmissible(connection)`
...
- [ ] T024 [US1] Delete `subscribe`'s `this.connections.set(connection.id, connection)` in
...
- [ ] T025 [US1] Correct `subscribe`'s comments about the deleted write in `packages/realtime/manager.ts` (FR-008).
  ...
  - Add `@throws {ConnectionNotRegisteredError}` to `subscribe`: raised at the pre-site, always before the
    authorizer.
- [ ] T026 [US1] Run the witness file. **W1, W2 and W9 must be green.** Run the realtime suite. The only failures
...
- [ ] T027 [US2] Widen `#assertAdmissible`'s clause 2 in `packages/realtime/manager.ts` to
...
- [ ] T028 [US2] Widen `ConnectionIdInUseError` in `packages/realtime/manager.ts` (FR-011). **Row 6.**
...
- [ ] T029 [US2] Update `register`'s JSDoc in `packages/realtime/manager.ts`. **Row 9's home for duty 1** (call it from
...
- [ ] T030 [US2] Run the witness file. **W3, W4, W6 and W8 must be green.** `#361` W8 stays green unmodified. Then run
...
- [ ] T031 [US3] Add `#isOwner(connection: Connection<Identity>): boolean` in `packages/realtime/manager.ts`, beside
...
- [ ] T032 [US3] Widen `disconnect` to `disconnect(target: string | Connection<Identity>)` in
...
- [ ] T033 [US3] Guard `disconnect`'s `finally` in `packages/realtime/manager.ts` (FR-009b). **Row 16.**
...
- [ ] T034 [US3] Update `handlerHooks` in `packages/realtime/manager.ts` (FR-009c). **Row 10's home.**
...
- [ ] T035 [US3] Run the witness file. **W5, W11, W12 (i and ii) and W13 must be green.** Then run the realtime suite.
...
- [ ] T036 [US4] Confirm without code changes:
...
- [ ] T037 [US4] Update the `Connection` JSDoc in `packages/realtime/types.ts` (FR-017). **Row 9's home for duty 2.**
  ...
  - `Connection.id`: the server mints the id per socket, never from client input and never from a user or session key
    (S2).
- [ ] T038 [US4] Give the `ChannelManager` class `@example` in `packages/realtime/manager.ts` its missing
...
- [ ] T039 [US5] Run W7. It goes green through T027: clause 2 now refuses the live holder's id, and `onOpen`'s close
  path is unchanged.
  - Confirm that the close reason is exactly `'unusable connection id'` (D5), and that the app's `onOpen` ran once.
- [ ] T040 **Commit 2.** Run `deno fmt`, `deno lint` and
...
- [ ] T041 Rewrite each test on T016's list, in its own file, to the new outcome. An example: a subscribe on an
...
- [ ] T042 Run `deno test -A packages/realtime/`: it is fully green. Commit as
  `test(370): rewrite the tests that relied on implicit registration`.

## Phase 10: Batteries (commit 4, `test(370)`)

- [ ] T043 New battery `packages/realtime/tests/mutations/register_only_admission_370.ts` (FR-015). `SUITES` is
...
- [ ] T044 [P] Re-anchor the rows whose anchor included the deleted `connections.set` (plan §4):
...
- [ ] T045 [P] Re-anchor the #361 rows in `packages/realtime/tests/mutations/disconnect_admission_361.ts`:
...
- [ ] T046 Re-run each battery holding an **unchanged adjacent row**, and confirm each row is `KILLED`:
  ...
  byte-identical.
- [ ] T047 If #376 has landed (T001), re-run `apply_revocation_376` and `revoke_channel_idless_340`, which must be
  clean.
- [ ] T048 Run `deno task mutate realtime` and compare it with T003. The new battery is added, and every battery is
...
- [ ] T049 [P] Amend ADR 010 in `docs/adr/010-realtime-disconnect-retires-the-connection-object.md` (FR-018). **Rows 12
...
- [ ] T050 [P] `docs/realtime.md`, the lifecycle section (`:503–551`) and the examples. **Row 9's user-facing home.**
...
- [ ] T051 `docs/realtime.md` § *Upgrading to v0.4.0*: correct **item 17's** `ConnectionIdInUseError` bullet to "a
  different object holds this id, live or being torn down". **Row 11.** This runs after T050, because it edits the
  same file.
- [ ] T052 `docs/realtime.md`: add **item 20, "`subscribe` requires `register`"**, numbered at landing (see the
...
- [ ] T053 `docs/realtime.md`: add **item 21, "An id held by a live connection is refused"**, numbered at landing.
  **Row 11's home.**
  ...
  - **The intro:** "Nineteen items. Thirteen are breaking changes — …" becomes "Twenty-one items. Fifteen are
    ...
    items …" list.
- [ ] T054 [P] `packages/realtime/README.md`: in the "A disconnected connection is refused at admission" bullet, widen
...
- [ ] T055 [P] `packages/realtime/AGENTS.md` (FR-020).
...
- [ ] T056 Hygiene greps, each checked by its count:
...
- [ ] T057 Commit T049–T055 as `docs(370,363): register-only admission and owner-scoped teardown`.
