...
- [X] T002 [US1] Add `driver?: 'postgres' | 'mysql' | 'sqlite'` to `DatabaseConfig` in `packages/core/kernel/kernel_decorators.ts` (optional, JSDoc, backward-compatible; absent = postgres). Regenerate core reexport baseline if the surface changes.
- [X] T003 [US1] In `packages/drizzle/mod.ts`: `Dialect` type + `DialectDatabase<D>` conditional type; make `Database<D extends Dialect = 'postgres'>` with `db: DialectDatabase<D>`; a `resolveDialect(config?, url)` resolver (single home, precedence per FR-002); an injectable per-dialect driver-factory seam (default = real dynamic imports with **fixed-literal** specifiers `import('drizzle-orm/postgres-js')`/`import('drizzle-orm/mysql2')`+`import('mysql2')`/`import('drizzle-orm/libsql')`+`import('@libsql/client')`); `connect()` branches by dialect; the catch renders via `renderError()` from `@lockness/contract`; a specific missing-client error (FR-004).
- [X] T004 [US1] Add `mysql2` + `@libsql/client` to `packages/drizzle/deno.json` imports (pinned). No deps.policy change (npm, not a workspace edge). `deno cache` to update the lockfile (never hand-edit).
- [X] T005 [US1] Update `packages/core/kernel/bootstrap/steps/database.ts` to pass `config.driver` into `connect` (FR-002a); confirm the CLI `initDatabase` path resolves via URL inference.
- [X] T006 [US1] Fast gate; commit `feat(T01): MySQL + SQLite drivers behind the Database service (#215)` + `Epic: #214`.
