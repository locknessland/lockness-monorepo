...
| The plugin set Lockness returns to Vite | `src/lockness.ts` — the `lockness()` factory (aggregate root); `src/mod.ts` is a barrel that re-exports it (home reconciled to shipped code, #155) | assembling the plugin array in `demo/vite.config.ts` instead of calling `lockness()` |
...
`plugins/deno.ts` (the resolver), `plugins/client_entry.ts`, `lockness.ts` (the
`lockness()` aggregate root), `build_config.ts` (the production build config
plugin), and `mod.ts` (the barrel that re-exports the public surface). _(As
shipped: `lockness.ts` and `build_config.ts` are their own modules and `mod.ts`
is a pure barrel — reconciled to code, #155.)_
...
- **New public package `@lockness/vite`** — exported surface. The **primary**
  surface, consumed by a user's `vite.config.ts` and server view code:
  `lockness()`, `defineViteConfig()`, `viteAssets()`, `LocknessViteConfig`,
  `ViteAssetsTagResult`, `DEFAULTS`. As shipped, `mod.ts` also exports an
  **advanced-use** surface (recorded here as intentional, #155 — its own header
  documents it as such): the individual plugin factories `lockness()` composes
  (`denoResolver`, `devServerBridge`, `clientEntry`, `cssPlugin`,
  `createCssCollector`, `hmrPlugin`, `buildConfigPlugin`) and `ManifestReader`,
  plus their types. Truly-internal helpers (`encodeAttribute`, `classifyChange`,
  `isValidSpecifier`, …) stay in `src/` only, off the semver surface.
