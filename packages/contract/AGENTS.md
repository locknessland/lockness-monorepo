...
| **Must never import**                          | `auth`, `auth-provider`, `cache`, `cli`, `container`, `core`, `crypto`, `devtools`, `drizzle`, `events`, `init`, `logger`, `notification`, `openapi`, `queue`, `redis`, `session`, `socialite`, `sse`, `telemetry`, `testing` — each already reaches this package, so importing one closes a cycle |
