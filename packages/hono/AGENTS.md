...
| **Must never import**                          | `auth`, `auth-provider`, `cache`, `cli`, `container`, `contract`, `core`, `crypto`, `devtools`, `drizzle`, `events`, `i18n`, `inertia`, `init`, `logger`, `mail`, `markdown`, `notification`, `openapi`, `queue`, `realtime`, `redis`, `session`, `socialite`, `sse`, `telemetry`, `testing`, `ui`, `validator` — each already reaches this package, so importing one closes a cycle |
