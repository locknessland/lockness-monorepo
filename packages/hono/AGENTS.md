...
| **Must never import**                          | `auth`, `auth-provider`, `cache`, `cli`, `container`, `contract`, `core`, `devtools`, `drizzle`, `events`, `inertia`, `init`, `logger`, `markdown`, `openapi`, `queue`, `redis`, `session`, `socialite`, `sse`, `testing`, `ui`, `validator` — each already reaches this package, so importing one closes a cycle |
