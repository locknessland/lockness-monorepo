...
| **Must never import**                          | `auth`, `auth-provider`, `cache`, `cli`, `container`, `contract`, `core`, `crypto`, `devtools`, `drizzle`, `events`, `inertia`, `init`, `logger`, `markdown`, `notification`, `openapi`, `queue`, `redis`, `session`, `socialite`, `sse`, `telemetry`, `testing`, `ui`, `validator` — each already reaches this package, so importing one closes a cycle |
