...
| **Must never import**                          | `auth`, `auth-provider`, `cli`, `core`, `devtools`, `drizzle`, `init`, `openapi`, `queue`, `session`, `testing` — each already reaches this package, so importing one closes a cycle |
