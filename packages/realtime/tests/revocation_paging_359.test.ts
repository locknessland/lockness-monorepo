...
/**
 * {@link RecordingRevocationDriver} with channel watching, whose unwatch
 * suspends until the test opens it — the one hook that holds a `disconnect`
 * inside its teardown loop.
 */
class GatedUnwatchDriver extends RecordingRevocationDriver {
    #open!: () => void
    ...
    watchChannel(_channel: string): void {}

    unwatchChannel(_channel: string): Promise<void> {
        this.#reach()
        return this.#gate
    }

    /** Let every suspended unwatch finish. */
    openUnwatch(): void {
        this.#open()
    }
}

Deno.test('#359 R13 (d) an id absent from connections is not this instance’s to apply, even while a stranded membership still names it: its record is not cleared', async () => {
    // `connections` is the manager's one spelling of ownership, and its
    // re-check applies a record only for an id in it (plan §5, the decider;
    // `owns` is only the driver being asked early). Every other state is
    // keyed by that same id, so for an id absent from all of them applying is
    // a no-op — except in the one state built here, where the id is gone from
    // `connections` but a membership still names it. There, applying would
    // leave the room and CLEAR the durable record; the decider must skip it.
    //
    // The state: a `subscribe` whose authorizer resolves while a `disconnect`
    // of the same id is suspended in its teardown loop re-adds the id and a
    // membership, and the disconnect's `finally` then forgets the id — the
    // membership stays. That leak is its own defect, #361, and is NOT
    // asserted as desired here: only that a record for an id this instance
    // does not own is left to its owner or its TTL. If #361 is fixed, the
    // precondition below fails; there is then no reachable state where
    // deleting the check is wrong, and M14 belongs in `expectSurvival` with
    // this reasoning.
    const driver = new GatedUnwatchDriver()
    let admit!: (verdict: PresenceMember | false) => void
    const manager = new ChannelManager<User>({
        driver,
        authorize: (user, channel) =>
            channel === ROOM
                ? new Promise<PresenceMember | false>((resolve) =>
                    admit = resolve
                )
                : authorize(user),
    })
    ...
    try {
        ...
        await driver.unwatching // the disconnect is inside its loop
        admit(authorize({ id: 1 }) as PresenceMember)
        assert((await subscribing).ok)
        driver.openUnwatch()
        await disconnecting
        ...
        assert(
            !state.connections.has('c1') &&
                state.subscriptions.get(ROOM)?.has('c1') === true,
            'precondition: c1 is gone from connections, a membership remains',
        )
        driver.ignoreOwns = true
        driver.revocations = [{ target: 'c1', channel: ROOM, id: 'r1' }]
        await driver.reconcile?.()
        assertEquals(
            driver.revocations.map((r) => r.id),
            ['r1'],
            'the record of an id this instance does not own is not cleared',
        )
        assertEquals(warnings.lines, [])
    } finally {
        warnings.restore()
