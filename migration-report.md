...
| 3 | `.specnaut/backlog-config.yml:1` | Provenance. The file *was* generated by `specflow init` and moved by `specnaut upgrade`; the comment now states both. Rewriting it to say `specnaut init` would falsify a record rather than fix a stale rule. |
