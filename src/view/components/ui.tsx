...
export const Card = (
    { children, title, description, class: className = '' }: {
        children: Child
        title?: string
        description?: string
        class?: string
    ...
    return (
        <div
            class={`bg-card border border-border rounded-3xl p-8 shadow-sm hover:border-primary/50 transition-colors group ${className}`}
        >
            {title && (
                <h3 class='text-2xl font-bold mb-2 text-card-foreground'>
                    {title}
                </h3>
            )}
            {description && (
