...
Decorators associate classes with required metadata and enable Lockness to create a
...
Lockness to create a handler for a specific endpoint for HTTP requests. The
...
route ( `todo`), and haven't added any path information in the decorator, Lockness
...
In our example above, when a GET request is made to this endpoint, Lockness routes
...
the route to, but Lockness doesn't attach any significance to the method name
chosen.
...
Handlers often need access to the client **request** details. Lockness provides
access to the
[request object](https://hono.dev/api/request).
We can access the request object by instructing Lockness to inject it by adding the
`@Req()` decorator to the handler's signature.

```ts todo.controller.ts
import { Controller, Get, Req } from 'jsr:@lockness/core';
...
```ts todo.controller.ts
import { Controller, Get, Post } from 'jsr:@lockness/core';
...
It's that simple. Lockness provides decorators for almost all of the standard HTTP
...
::: info Hint
Import `Param` from the `jsr:@lockness/core` package.
:::

## Scopes

For people coming from different programming language backgrounds, it might be
unexpected to learn that in Lockness, almost everything is shared across incoming
...
**asynchronous**. That's why Lockness supports and works well with `async`
functions.
...
deferred value that Lockness will be able to resolve by itself. Let's see an
example of this:
...
Lockness uses [Validatte](https://github.com/Savory/validatte) to validate the Dto Schema with
...
import { IsNumber, IsString, IsHexColor } from 'jsr:@lockness/validatte';
...
::: info Hint
More details about Body validation can be found [here](https://savory.github.io/body-validation-in-lockness/).
:::
...
With the above controller fully defined, Lockness still doesn't know that
...
```ts app.module.ts
import { Module } from 'jsr:@lockness/core';
...
and Lockness can now easily reflect which controllers have to be mounted.

Now, it is time to create a LocknessApplication that bootstrap our AppModule.

We advise you to create a bootstrap function that returns your LocknessApplication instance, this will make testing easier as you can get your application instance and make it listen to a random port.

```ts bootstrap.ts
import { AppModule } from './app.module.ts';
import { LocknessApplication } from 'jsr:@lockness/core';
