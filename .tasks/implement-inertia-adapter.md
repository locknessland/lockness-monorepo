...
export interface PageObject {
     ...
     * Includes page-specific props, shared props, and always includes `errors` (defaults to {}).
     */
    readonly props: Record<string, unknown> & {
        errors?: Record<string, string>
    }
    ...
    /**
     * The current page URL.
     * Used by the client for navigation state and browser history.
     */
    readonly url: string

    /**
     * The current asset version.
     * Used for cache invalidation via X-Inertia-Version header comparison.
     */
    readonly version: string
 ...
 * Props that can be passed to inertia.render().
 * Values can be:
 * - Raw data (any serializable value)
 * - Lazy functions (resolved only when needed)
 * - Async lazy functions (for database queries, etc.)
 */
export type InertiaProps = Record<
    string,
    unknown | (() => unknown) | (() => Promise<unknown>)
...
export class Inertia {
     ...
     * Render an Inertia response.
     *
     * Returns JSON for Inertia requests (XHR) or HTML for initial page loads.
     *
     * @param component - The name of the page component to render
     * @param props - Props to pass to the component
     * @param options - Additional render options (encryptHistory, clearHistory)
    ...
    async render(
        ...
        // Resolve lazy props (skip for partial reloads if not requested)
        ...
        // Build page object per Inertia protocol spec
        ...
        if (this.isInertiaRequest()) {
            ...
            this.context.header('Vary', 'X-Inertia') // Per protocol spec
...
Deno.test('Inertia.render - returns JSON for Inertia requests', async () => {
    ...
    assertEquals(res.headers.get('Content-Type'), 'application/json')
    assertEquals(res.headers.get('X-Inertia'), 'true')
    assertEquals(res.headers.get('Vary'), 'X-Inertia') // Per protocol spec

    const json = await res.json()
    assertEquals(json.component, 'TestComponent')
    assertEquals(json.props.message, 'Hello')
    assertEquals(json.props.errors, {}) // Always includes errors object
    assertEquals(json.version, '1.0')
    assertEquals(json.encryptHistory, false)
    assertEquals(json.clearHistory, false)
})
...
## 📖 Inertia.js Protocol Reference (Adapted for Lockness)

This section provides a comprehensive reference of the Inertia.js protocol,
adapted with Lockness examples instead of the Laravel examples found in the
official documentation.

> 📚 **Source**: Official Inertia.js LLM documentation:
> https://inertiajs.com/docs/llms.txt

### The Protocol

#### HTML Responses (First Page Load)

The first request to an Inertia app is a regular full-page browser request. The
server returns a full HTML document with a root `<div>` containing a `data-page`
attribute with the JSON-encoded page object.
...
<html>
    ...
    <body>
        <div id="app" data-page='{"component":"Event/Show","props":{"event":{"id":80,"title":"Birthday party"}},"url":"/events/80","version":"c32b8e4965f418ad16eaebba1d4e960f"}'></div>
    </body>
</html>
```

**Lockness Implementation:**

```typescript
// packages/inertia/helpers.ts
export function defaultRootView(page: PageObject): string {
    const pageJson = serializePageForHtml(page)
...
#### JSON Responses (Inertia XHR Requests)

Subsequent requests include the `X-Inertia: true` header. The server responds
...
The page object contains all data needed to render a page:

| Property         | Description                                     |
| ---------------- | ----------------------------------------------- |
| `component`      | The JavaScript page component name              |
| `props`          | Page props (data), always includes `errors: {}` |
| `url`            | The page URL                                    |
| `version`        | Current asset version                           |
| `encryptHistory` | Whether to encrypt history state (optional)     |
| `clearHistory`   | Whether to clear encrypted history (optional)   |

#### Asset Versioning (409 Conflict)

When asset versions mismatch, the server returns a `409 Conflict` with the
...
X-Inertia: true
X-Inertia-Version: old-version-hash
...
```typescript
// packages/inertia/middleware.ts
if (isInertiaRequest(c)) {
    const clientVersion = c.req.header('X-Inertia-Version')
    if (clientVersion && clientVersion !== currentVersion) {
        return new Response(null, {
...
For PUT/PATCH/DELETE requests, 302 redirects must be converted to 303 to ensure
...
Share data that should be available on every page (auth user, flash messages).

**Lockness Implementation:**

```typescript
// kernel.tsx - Share global props
app.useMiddleware(async (c, next) => {
    const inertia = c.get('inertia')
    ...
    inertia.share({
        ...
        appName: 'My Lockness App',

        // Auth user (lazily evaluated)
        auth: {
            user: user
...
Wrap expensive operations in functions for deferred evaluation.
...
export class UserController {
    ...
    async show(c: Context) {
        ...
        return inertia.render('Users/Show', {
            // ALWAYS included, ALWAYS evaluated
            user: await this.userService.findById(id),
            ...
            // Async lazy props work too
            stats: async () => await this.userService.getStats(id),
        })
    }
}
```
...
Inertia handles validation differently than traditional XHR forms. Instead of
returning 422 responses, you redirect back with errors in the session.
...
export class UserController {
    ...
    async store(c: Context) {
        ...
        if (!validation.passes()) {
            ...
            return c.redirect(c.req.header('Referer') ?? '/users/create', 303)
        }
...
```typescript
// Middleware to share errors from session
app.useMiddleware(async (c, next) => {
    const inertia = c.get('inertia')
    const session = c.get('session')

    // Always share errors (empty object if none)
    inertia.share({
...
```tsx
// React component displaying errors
...
### Partial Reloads (Phase 2)

Request only specific props on subsequent visits to the same page.
...
**Lockness Implementation (Phase 2):**

```typescript
// In Inertia.render() - check for partial reload headers
...
if (partialComponent === component && partialData) {
    const requestedKeys = partialData.split(',')
    // Only resolve and return requested props
    resolvedProps = await this.resolvePartialProps(props, requestedKeys)
}
```

### External Redirects

For redirects to external URLs or non-Inertia endpoints, use a 409 response with
`X-Inertia-Location`.

**Lockness Implementation:**

```typescript
// packages/inertia/inertia.ts
export class Inertia {
    /**
     * Redirect to an external URL or non-Inertia page.
     * Triggers a full page reload on the client.
     */
    location(url: string): Response {
        ...
        return this.context.redirect(url, 302)
    }
}
...
```typescript
// app/controller/event_controller.tsx
import {
    type Context,
    Controller,
    Delete,
...
import { Inject } from '@lockness/container'
import { EventService } from '../service/event_service.ts'
...
export class EventController {
    ...
    async create(c: Context) {
        ...
        return inertia.render('Events/Create', {
            categories: await this.eventService.getCategories(),
        })
    }
    ...
    async update(c: Context) {
        ...
        // 303 redirect for PUT/PATCH/DELETE
