...
2. Delegate structural review to parallel sub-agents via the
   `review-coordinator`.
3. Detect the project's toolchain and run its quality gates.
4. If CRITICAL or HIGH findings exist, route fixes to the implementer and
   re-run.
...
file:line references and a suggested fix. The coordinator aggregates them into a
single report.
