...
- For each executable hook, output the following based on its `optional` flag:
  ...
  - **Mandatory hook** (`optional: false`):
    ...
    **Automatic Pre-Hook**: {extension}
    ...
    ```
- If no hooks are registered or `.specflow/extensions.yml` does not exist, skip
  silently

## Outline

You are updating the project constitution at `.specflow/memory/constitution.md`.
...
**Note**: If `.specflow/memory/constitution.md` does not exist yet, it should
have been initialized from `.specflow/templates/constitution-template.md` during
...
4. Consistency propagation checklist (convert prior checklist into active
   validations):
   - Read `.specflow/templates/plan-template.md` and ensure any "Constitution
     Check" or rules align with updated principles.
   - Read `.specflow/templates/spec-template.md` for scope/requirements
     alignment—update if constitution adds/removes mandatory sections or
     constraints.
   - Read `.specflow/templates/tasks-template.md` and ensure task categorization
     reflects new or removed principle-driven task types (e.g., observability,
     versioning, testing discipline).
   - Read each command file in `.specflow/templates/commands/*.md` (including
...
7. Write the completed constitution back to `.specflow/memory/constitution.md`
   (overwrite).
...
`.specflow/memory/constitution.md` file.

## Post-Execution Checks

**Check for extension hooks (after constitution update)**: Check if
`.specflow/extensions.yml` exists in the project root.
