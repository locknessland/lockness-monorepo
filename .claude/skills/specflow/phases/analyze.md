...
implementation. This command MUST run only after `/specflow tasks` has
...
update outside `/specflow analyze`.
...
- Provide explicit command suggestions: e.g., "Run /specflow specify --manual to
  refine the spec without re-cascading", "Run /specflow plan --once to
  regenerate the plan only, or /specflow plan to cascade through tasks →
  review", "Manually edit tasks.md to add coverage for 'performance-metrics'"
