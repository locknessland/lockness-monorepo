...
3. **Agent context update**:
   - Update the plan reference between the `<!-- SPECFLOW START -->` and
     `<!-- SPECFLOW END -->` markers in `__CONTEXT_FILE__` to point to the plan
     file created in step 1 (the IMPL_PLAN path)
...
- Use absolute paths for filesystem operations; use project-relative paths for
  references in documentation and agent context files
