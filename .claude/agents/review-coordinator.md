...
**This block is restated in full here**, unlike every other seat's, because its
fields carry aggregation semantics the contract does not: `summed across seats`,
`the union of the seats' EVIDENCE`, and a `SEATS_REPORTED` that counts seats
which reported **and, when clean, showed evidence**. The verdict rule below adds
the two aggregation triggers to the contract's `SEATS_REPORTED == SEATS_EXPECTED`
clause. If the contract's rule changes, this is the one file to reconcile by hand.
...
**Required seats:** `code-reviewer` and `security-expert`, always.
`test-reviewer` is required **only when the diff contains test files** — when it
does not, it is not spawned and not counted in `SEATS_EXPECTED`. That is
`SKIPPED`, and a skipped seat is not a missing one.
...
- **Count it.** `SEATS_EXPECTED` is the number of required seats;
  ...
  older than this contract, not a seat that did not look. The verdict then falls
  out of the contract's arithmetic rather than a prose rule fighting the count.
- Name it in `TOP_ISSUES` on its own line.

### A clean seat report is checked against the diff, not taken on its word

Everything above counts blocks. A seat emitting `1`/`1` with all-zero counts and
an empty report is indistinguishable from one that looked. So a **clean** report
(every severity count `0`) gets one check that does not go through the seat's own
count:
...
- `EVIDENCE` absent, empty, `NONE`, or naming nothing from the diff → that seat
  ...
  block.
- A seat **with findings** is exempt. Its findings name their own locations, and
  demanding a second list would fail honest reports over a formatting miss.
...
- If you substitute your own checking for the missing seat, **label it as
  yours**. Coordinator verification is not a seat report.
...
`CRITICAL_COUNT == 0` **and** `HIGH_COUNT == 0`; `fail` when either count is > 0
**or** `SEATS_REPORTED < SEATS_EXPECTED`; `needs_followup` when only Medium/Low
remain. The `SEATS_REPORTED < SEATS_EXPECTED` trigger is what gives an all-zero
run with a missing seat a verdict. Then emit the `WORKFLOW STATUS` block per
`workflow-contract` and, when handing the gate result back, the `HANDOFF` block
