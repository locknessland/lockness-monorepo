...
`)
    ...
    cli.register('make:action', async (args) => {
        ...
        if (!controllerName || !actionName) {
            ...
            console.error('   Options: --method=get|post|put|delete|patch (default: get)')
        ...
        if (!['get', 'post', 'put', 'delete', 'patch'].includes(method)) {
            console.error('❌ Invalid method. Use: get, post, put, delete, or patch')
            return
        }

        const className = controllerName.charAt(0).toUpperCase() + controllerName.slice(1)
        const controllerFileName = `${controllerName.toLowerCase()}_controller.tsx`
        ...
        try {
            ...
            if (withView) {
                ...
                const viewFileName = `${controllerName.toLowerCase()}/${actionName.toLowerCase()}`
                const viewDirPath = `./src/view/pages/${controllerName.toLowerCase()}`
                const viewFilePath = `${viewDirPath}/${actionName.toLowerCase()}.tsx`
            ...
            } else if (actionName === 'show' || actionName === 'edit' || actionName === 'update' || actionName === 'destroy') {
                body = `const id = c.req.param('id')\n        return c.json({ message: '${actionName} ${className} ' + id })`
            } else if (actionName === 'store' || actionName === 'update') {
                body = `const body = await c.req.json()\n        return c.json({ message: '${className} ${actionName}d', data: body })`
            } else {
                body = `return c.json({ message: '${actionName} from ${className}Controller' })`
            }
            ...
            if (!controllerContent.includes(decoratorName) && decoratorImports.includes(decoratorName)) {
                // Add to imports
                finalContent = finalContent.replace(
                    ...
                    (match, imports) => {
                        ...
                        return `import { ${importList.join(', ')} } from 'lockness'`
                    }
                )
            }
            ...
            if (withView) {
                const viewClassName = `${className}${actionName.charAt(0).toUpperCase() + actionName.slice(1)}`
                const viewImport = `import { ${viewClassName} } from '@view/pages/${controllerName.toLowerCase()}/${actionName.toLowerCase()}.tsx'\n`
            ...
            console.log(`   Route: ${method.toUpperCase()} ${path} → ${routeName}`)

        } catch (error) {
            if (error instanceof Deno.errors.NotFound) {
                console.error(`❌ Controller not found: ${controllerPath}`)
                console.error('   Create it first with: make:controller ${controllerName}')
            } else {
