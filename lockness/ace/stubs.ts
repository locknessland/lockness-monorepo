...
export class Stub {
     ...
     */
    static async renderFrom(baseDir: string, type: string, name: string, data: Record<string, string> = {}): Promise<string> {
        ...
        try {
            ...
            throw new Error(`Could not find stub at ${stubPath}: ${(error as Error).message}`);
        }
    }
     ...
     */
    static async render(type: string, name: string, data: Record<string, string> = {}): Promise<string> {
     ...
     */
    static async scaffold(type: string, targetDir: string, data: Record<string, string> = {}) {
        ...
        const walk = async (currentSource: string, currentTarget: string) => {
            ...
            for (const entry of entries) {
                ...
                const targetPath = join(currentTarget, entry.name.replace('.stub', ''));
